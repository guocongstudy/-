 ...
 * curl "http://localhost:8081/set?uid=1&money=100&num=100"
 * 2 抢红包，先到先得，随机得到红包金额
 * curl "http://localhost:8081/get?uid=1&id=2556060954" 后面的255...是红包id需要改的
 * 注意：
 * 线程不安全1，红包列表 packageList map 的并发读写会产生异常
 * 测试方法： wrk -t10 -c10 -d5  "http://localhost:8081/set?uid=1&money=100&num=10"
